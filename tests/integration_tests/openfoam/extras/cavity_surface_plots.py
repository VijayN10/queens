...
"""
    ...
    1. lid_velocity vs initial_pressure vs Ux
    2. lid_velocity vs initial_pressure vs Uy  
    3. lid_velocity vs initial_pressure vs pressure

    Create 2D pressure sensitivity plots
...
def create_training_vs_predicted_plot_2d(surrogate_model, param_ranges, probe_idx=0):
    ...
    press_vals = np.linspace(param_ranges['min'][1], param_ranges['max'][1], resolution)
    VEL, PRESS = np.meshgrid(vel_vals, press_vals)
    X_grid = np.column_stack([VEL.ravel(), PRESS.ravel()])
    ...
    ax1.set_title(f'Ux vs Pressure - {probe_name}')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    plt.colorbar(scatter1_pred, ax=ax1, label='Lid Velocity')
    
    # Plot 2: Uy vs Pressure
    ...
    scatter2_train = ax2.scatter(X_train[:, 1], uy_train, c=X_train[:, 0], 
                                ...
                                edgecolors='black', linewidth=1, label='Training')
    ax2.set_xlabel('Initial Pressure (Pa)')
    ax2.set_ylabel('Uy (m/s)')
    ax2.set_title(f'Uy vs Pressure - {probe_name}')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    plt.colorbar(scatter2_pred, ax=ax2, label='Lid Velocity')
    
    # Plot 3: Pressure vs Pressure
    ...
    scatter3_train = ax3.scatter(X_train[:, 1], pressure_train, c=X_train[:, 0], 
                                ...
                                edgecolors='black', linewidth=1, label='Training')
    ax3.set_xlabel('Initial Pressure (Pa)')
    ax3.set_ylabel('Output Pressure (Pa)')
    ax3.set_title(f'Pressure vs Pressure - {probe_name}')
    ax3.legend()
...
def create_3d_surface_plots(surrogate_model, param_ranges, probe_idx=0, resolution=30):
    ...
    surf1 = ax1.plot_surface(VEL, PRESS, UX, cmap='viridis', alpha=0.8)
    ax1.set_xlabel('Lid Velocity')
    ax1.set_ylabel('Initial Pressure')
    ...
    surf2 = ax2.plot_surface(VEL, PRESS, UY, cmap='plasma', alpha=0.8)
    ax2.set_xlabel('Lid Velocity')
    ax2.set_ylabel('Initial Pressure')
    ...
    surf3 = ax3.plot_surface(VEL, PRESS, P, cmap='coolwarm', alpha=0.8)
    ax3.set_xlabel('Lid Velocity')
    ax3.set_ylabel('Initial Pressure')
...
def create_individual_surface_plots(surrogate_model, param_ranges, probe_idx=0, resolution=40):
    ...
    surf1 = ax1.plot_surface(VEL, PRESS, UX, cmap='viridis', alpha=0.9, edgecolor='none')
    ax1.set_xlabel('Lid Velocity (m/s)', fontsize=12)
    ax1.set_ylabel('Initial Pressure (Pa)', fontsize=12)
    ...
    surf2 = ax2.plot_surface(VEL, PRESS, UY, cmap='plasma', alpha=0.9, edgecolor='none')
    ax2.set_xlabel('Lid Velocity (m/s)', fontsize=12)
    ax2.set_ylabel('Initial Pressure (Pa)', fontsize=12)
    ...
    surf3 = ax3.plot_surface(VEL, PRESS, P, cmap='coolwarm', alpha=0.9, edgecolor='none')
    ax3.set_xlabel('Lid Velocity (m/s)', fontsize=12)
    ax3.set_ylabel('Initial Pressure (Pa)', fontsize=12)
...
def plot_pressure_sensitivity_2d(surrogate_model, param_ranges):
    """Create 2D plots showing pressure sensitivity for each probe and variable."""
    
    # Fixed lid velocity at middle of range
    fixed_lid_vel = (param_ranges['min'][0] + param_ranges['max'][0]) / 2
    
    # Pressure range
    pressure_vals = np.linspace(param_ranges['min'][1], param_ranges['max'][1], 100)
    X_pressure_sweep = np.column_stack([np.full(len(pressure_vals), fixed_lid_vel), pressure_vals])
    
    # Get predictions
    pred_dict = surrogate_model.predict(X_pressure_sweep)
    Y_pred = pred_dict['result']
    ...
    for probe_idx in range(5):
        ...
        # Ux vs pressure
        ax1.plot(pressure_vals, ux, 'b-', linewidth=2, marker='o', markersize=3)
        ax1.set_xlabel('Initial Pressure')
        ax1.set_ylabel('Ux (m/s)')
        ax1.set_title(f'Ux vs Pressure - {probe_names[probe_idx]}')
        ax1.grid(True, alpha=0.3)
        
        # Uy vs pressure  
        ax2.plot(pressure_vals, uy, 'r-', linewidth=2, marker='o', markersize=3)
        ax2.set_xlabel('Initial Pressure')
        ax2.set_ylabel('Uy (m/s)')
        ax2.set_title(f'Uy vs Pressure - {probe_names[probe_idx]}')
        ax2.grid(True, alpha=0.3)
        
        # Pressure vs pressure
        ax3.plot(pressure_vals, pressure_out, 'g-', linewidth=2, marker='o', markersize=3)
        ax3.set_xlabel('Initial Pressure')
        ax3.set_ylabel('Output Pressure (Pa)')
        ax3.set_title(f'Pressure vs Pressure - {probe_names[probe_idx]}')
        ax3.grid(True, alpha=0.3)
        
        plt.suptitle(f'Pressure Sensitivity - {probe_names[probe_idx].replace("_", " ").title()} Probe (lid_vel={fixed_lid_vel:.2f})')
        plt.tight_layout()
        
        fig.savefig(f"plots/pressure_sensitivity_{probe_names[probe_idx]}_probe_{probe_idx}.png", 
                   dpi=300, bbox_inches='tight')
        print(f"✅ Pressure sensitivity plot saved for {probe_names[probe_idx]}")
    
    print("✅ All pressure sensitivity plots created!")
...
def plot_lid_velocity_sensitivity_2d(surrogate_model, param_ranges):
   ...
   # Fixed pressure at middle of range
   fixed_pressure = (param_ranges['min'][1] + param_ranges['max'][1]) / 2
   
   # Lid velocity range
   velocity_vals = np.linspace(param_ranges['min'][0], param_ranges['max'][0], 100)
   X_velocity_sweep = np.column_stack([velocity_vals, np.full(len(velocity_vals), fixed_pressure)])
   ...
   for probe_idx in range(5):
       ...
       plt.suptitle(f'Lid Velocity Sensitivity - {probe_names[probe_idx].replace("_", " ").title()} Probe (pressure={fixed_pressure:.3f})')
       plt.tight_layout()
...
def debug_training_data():
    """Check if training data shows pressure-Ux relationship."""
    try:
        with open("surrogate_data_output/cavity_flow_surrogate_surrogate_data.pkl", "rb") as f:
            data = pickle.load(f)
        
        X_train = data['X_train']  # [lid_vel, pressure]
        Y_train = data['Y_train']  # probe outputs
        
        # Plot Ux vs pressure colored by lid velocity for center probe (probe 0)
        fig, ax = plt.subplots(figsize=(10, 6))
        scatter = ax.scatter(X_train[:, 1], Y_train[:, 0], c=X_train[:, 0], cmap='viridis', alpha=0.7)
        ax.set_xlabel('Initial Pressure')
        ax.set_ylabel('Ux at Center Probe (training data)')
        ax.set_title('Training Data: Ux vs Pressure (colored by lid velocity)')
        plt.colorbar(scatter, label='Lid Velocity')
        
        fig.savefig("plots/debug_training_data_ux_pressure.png", dpi=300, bbox_inches='tight')
        print("✅ Debug plot saved: debug_training_data_ux_pressure.png")
        
        # Print some statistics
        print(f"\nTraining data statistics:")
        print(f"Lid velocity range: {X_train[:, 0].min():.3f} to {X_train[:, 0].max():.3f}")
        print(f"Pressure range: {X_train[:, 1].min():.3f} to {X_train[:, 1].max():.3f}")
...
if __name__ == "__main__":
    ...
    # Create 2D pressure sensitivity plots
    print("\n🔍 CREATING 2D PRESSURE SENSITIVITY PLOTS...")
    surrogate_model, param_ranges = load_trained_model()
    plot_pressure_sensitivity_2d(surrogate_model, param_ranges)
